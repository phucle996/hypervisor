...
func (s *Server) AgentControlStream(stream agentregistryv1.AgentRegistry_AgentControlStreamServer) error {
	...
	nodeID, err := s.service.RegisterAgentHost(stream.Context(), entity.AgentRegistration{
		...
		CPUThreads:       int(registerMessage.RegisterHost.GetCpuThreads()),
		MemoryBytes:      registerMessage.RegisterHost.GetMemoryBytes(),
		DiskBytes:        registerMessage.RegisterHost.GetDiskBytes(),
		GPUCores:         int(registerMessage.RegisterHost.GetGpuCores()),
		GPUMemoryBytes:   registerMessage.RegisterHost.GetGpuMemoryBytes(),
		CPUModel:         registerMessage.RegisterHost.GetCpuModel(),
		RAMModel:         registerMessage.RegisterHost.GetRamModel(),
		DiskModel:        registerMessage.RegisterHost.GetDiskModel(),
		GPUModel:         registerMessage.RegisterHost.GetGpuModel(),
	})
	if err != nil {
		...
		case *agentregistryv1.AgentToHypervisor_NodeMetric:
			if err := s.service.RecordNodeMetric(stream.Context(), entity.NodeMetricIngest{
				...
				Metric: entity.HypervisorNodeMetric{
					...
					CPUUsedCores:   message.NodeMetric.GetCpuUsedCores(),
					RAMUsedGib:     message.NodeMetric.GetRamUsedGib(),
					RAMUsedPercent: message.NodeMetric.GetRamUsedPercent(),
					SSDUsedGib:     message.NodeMetric.GetSsdUsedGib(),
					SSDUsedPercent: message.NodeMetric.GetSsdUsedPercent(),
					GPUUsedGib:     message.NodeMetric.GetGpuUsedGib(),
					GPUUsedPercent: message.NodeMetric.GetGpuUsedPercent(),
					NetworkRxBps:   message.NodeMetric.GetNetworkRxBps(),
					NetworkTxBps:   message.NodeMetric.GetNetworkTxBps(),
					DiskReadBps:    message.NodeMetric.GetDiskReadBps(),
					DiskWriteBps:   message.NodeMetric.GetDiskWriteBps(),
		...
		case *agentregistryv1.AgentToHypervisor_VpsMetric:
			if err := s.service.RecordVPSMetric(stream.Context(), entity.VPSMetricIngest{
				...
				Metric: entity.VPSMetric{
					...
					CPUUsedCores:   message.VpsMetric.GetCpuUsedCores(),
					RAMUsedGib:     message.VpsMetric.GetRamUsedGib(),
					RAMUsedPercent: message.VpsMetric.GetRamUsedPercent(),
					SSDUsedGib:     message.VpsMetric.GetSsdUsedGib(),
					SSDUsedPercent: message.VpsMetric.GetSsdUsedPercent(),
					GPUUsedGib:     message.VpsMetric.GetGpuUsedGib(),
					GPUUsedPercent: message.VpsMetric.GetGpuUsedPercent(),
					NetworkRxBps:   message.VpsMetric.GetNetworkRxBps(),
					NetworkTxBps:   message.VpsMetric.GetNetworkTxBps(),
					DiskReadBps:    message.VpsMetric.GetDiskReadBps(),
					DiskWriteBps:   message.VpsMetric.GetDiskWriteBps(),
