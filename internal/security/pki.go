...
const agentCommonNamePrefix = "hypervisor-agent:"
...
func LoadCertificateAuthority(certPath, keyPath string) (*CertificateAuthority, error) {
	...
	return &CertificateAuthority{cert: cert, key: signer, certPEM: certPEM}, nil
}
...
func (ca *CertificateAuthority) SignAgentClientCertificate(csrPEM, agentID string, ttl time.Duration) (string, string, time.Time, error) {
	if ca == nil || ca.cert == nil || ca.key == nil {
		return "", "", time.Time{}, errors.New("security: certificate authority is not initialized")
	}
	if strings.TrimSpace(agentID) == "" {
		return "", "", time.Time{}, errors.New("security: agent id is empty")
	}
	if ttl <= 0 {
		ttl = 30 * 24 * time.Hour
	}
	...
	template := &x509.Certificate{
		...
		Subject: pkix.Name{
			CommonName:   AgentCommonName(agentID),
			Organization: []string{"aurora-hypervisor"},
		},
		NotBefore:             notBefore,
		NotAfter:              notAfter,
...
func AgentCommonName(agentID string) string {
	return agentCommonNamePrefix + strings.TrimSpace(agentID)
}
...
func AgentIDFromCertificate(cert *x509.Certificate) (string, error) {
	...
	if !strings.HasPrefix(commonName, agentCommonNamePrefix) {
		return "", errors.New("security: unexpected certificate common name")
	}

	agentID := strings.TrimPrefix(commonName, agentCommonNamePrefix)
	if strings.TrimSpace(agentID) == "" {
		return "", errors.New("security: empty agent id in certificate")
	}
	return agentID, nil
}
