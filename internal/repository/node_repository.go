...
SET hostname = $2,
    ...
    updated_at = NOW()
WHERE id = $1
`, nodeID, input.Hostname, entity.NodeStatusActive, input.PrivateIP, input.CPUCores, ramGiB, diskGiB, input.GPUCores, input.GPUMemoryBytes, string(nodeMetadata), input.CPUThreads, input.CPUModel, input.RAMModel, input.DiskModel, input.GPUModel)
	if err != nil {
